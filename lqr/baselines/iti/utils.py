...
# # Model aliases used for loading tokenizers/models and for filename-safe keys.
# MODEL_NAME_MAP = {
...
# }
# ENGINE_MAP = MODEL_NAME_MAP
# MODEL_FAMILIES = ("llama", "alpaca", "vicuna", "qwen", "gemma")

# def resolve_model_name_or_path(model_name, model_prefix="", mapping=None):
#     mapping = MODEL_NAME_MAP if mapping is None else mapping
#     key = f"{model_prefix}{model_name}"
#     return mapping.get(key, key)

def _submodule_exists(model, path):
    try:
...
def resolve_layers_prefix(model):
    ...
    raise ValueError("Unsupported model layout: could not find transformer layers")

def resolve_attn_out_proj_component(model):
    layers_prefix = resolve_layers_prefix(model)
    suffix="self_attn.o_proj"
    layer0_prefix = f"{layers_prefix}.0"
    # candidate_suffixes = [
    #     "self_attn.o_proj",
    #     "self_attn.out_proj",
    #     "attn.o_proj",
    ...
    #     if _submodule_exists(model, f"{layer0_prefix}.{suffix}"):
    #         print(f"{layer0_prefix}.{suffix}")
    #         return f"{layers_prefix}[{{layer}}].{suffix}.input"
    if _submodule_exists(model, f"{layer0_prefix}.{suffix}"):
        return f"{layers_prefix}[{{layer}}].{suffix}.input"
    raise ValueError("Unsupported model attention projection layout")
...
def get_llama_activations_pyvene(collected_model, collectors, prompt, device):
    ...
    # Preserve (num_collectors, seq_len, hidden_dim) even when seq_len == 1.
...
#     assert any(tag in model_key for tag in MODEL_FAMILIES), 'model must be llama/qwen/gemma family'
...
#         # llama-family / qwen / gemma
#         if any(tag in mdl for tag in MODEL_FAMILIES):
...
#         #     continue
#         if any(tag in model_key for tag in MODEL_FAMILIES):
