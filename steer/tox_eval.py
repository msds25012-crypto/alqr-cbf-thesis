from steer.toxicity.test_toxicity import PATH, load_file, run_trials
...
def ppl(models, params):
    for model_name in models:
        ...
        f_pfix = params[model_name]['filename_pfix']
        w = get_ppl_from_file(f_pfix + "_tox_eval", path=PATH)
        if w:
...
def mmlu(models, params):
    ...
    for model_name in models:
        ...
        out = test_mmlu(model, tokenizer, X_contr, A, lambda_list=l_list, q=q, r=r, qf=qf, N_PROMPTS=10, N_LOOP=1, BATCH_SIZE=4, N_SHOTS = 5, INSTRUCT=False)
...
def main():
    ...
    params = { 
        'google/gemma-2-2b': {'filename_pfix': 'gemma-2-2b', 
                              'l_list': [1, 1.25, 1.375, 1.5], 
                              'LQR_params': [0.1,3,0.1],
                              'instruct': False},
